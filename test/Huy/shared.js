...
async function navigateToRegister(page) {
  // Reset page state trước để tránh corrupted context từ redirect trước đó
...
const EXCEL_COLS = [{ wch: 14 }, { wch: 30 }, { wch: 16 }, { wch: 45 }, { wch: 25 }, { wch: 25 }, { wch: 60 }, { wch: 60 }, { wch: 10 }];
...
function extractErrorPhrases(text) {
  ...
  if (phrases.length === 0) {
    let cleaned = text.replace(/^(Hiển thị (lỗi|thông báo)[^:]*:\s*)/i, "").replace(/[.!]+$/g, "").trim();
...
async function runSingleTest(page, testCase, folderName) {
  ...
  try {
    ...
    // Dùng form.requestSubmit() thay vì page.click() để gửi form ổn định hơn
    await page.evaluate(() => {
      document.getElementById("registerForm").requestSubmit();
    });

    // Chờ navigation (nếu thành công → redirect) hoặc timeout (nếu lỗi)
    await Promise.race([

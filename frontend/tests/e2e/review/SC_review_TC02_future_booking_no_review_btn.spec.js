...
const DIR = path.resolve(__dirname, "screenshots", "TC02-future-booking-no-review-btn");
...
(async () => {
	...
	try {
		...
		// Bước 3: Vào chi tiết booking 88 (chưa khởi hành)
		console.log("\n========== BƯỚC 3: CHI TIẾT BOOKING ==========");
		await page.goto(`${BASE_URL}/pages/user/booking-details.html?id=88`, {
		...
		// Kiểm tra nút "Yêu cầu hủy" có hiển thị
		...
		if (cancelBtn) {
			...
			console.log(`  ✅ Nút "Yêu cầu hủy" hiển thị: "${btnText.trim()}"`);
		} else {
			console.log('  ⚠️ Không tìm thấy nút "Yêu cầu hủy"');
		}

		// Kiểm tra nút "Viết đánh giá" KHÔNG hiển thị
		const reviewBtn = await page.$("#booking-actions .btn-warning");
		if (!reviewBtn) {
			console.log('  ✅ Nút "Viết đánh giá" không hiển thị (đúng)');
		} else {
			console.log('  ⚠️ Nút "Viết đánh giá" vẫn hiển thị (sai)');
		}

		await shot("03-booking-detail-future");

		console.log("\n============================================");
		console.log(`✅ TC02 FUTURE BOOKING - ${step} ảnh đã lưu`);

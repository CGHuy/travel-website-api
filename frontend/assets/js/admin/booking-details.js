...
function renderAdminBookingDetails(data) {
	...
	setElText(
		"det-user-id",
		`USR${String(data.user_id).padStart(3, "0") || "KHÁCH"}`,
	);
	...
	setElText(
		"det-ret-date",
		formatDate(data.return_date || addDays(data.departure_date, 3)),
	);
	...
	if (priceBreakdown) {
		const adultPriceTotal =
			data.adults *
			(parseFloat(data.price_default) + parseFloat(data.price_moving));
		const childPriceTotal =
			data.children *
			(parseFloat(data.price_child) + parseFloat(data.price_moving_child));
	...
	setElText(
		"det-payment-meta",
		`Thanh toán qua: Chuyển khoản - ${formatDate(data.created_at)}`,
	);
	...
	if (cancelActionArea && data.status === "pending") {
		...
		approveBtn.onclick = async () => {
			...
			try {
				...
				// Không cập nhật status ở đây — backend vnpayRefundReturn sẽ tự cập nhật
				// status → cancelled + payment_status → refunded sau khi VNPay xác nhận thành công.
				...
				console.error(err);
				window.showToast(
					`Không thể thực hiện phê duyệt: ${err.message}`,
					"error",
				);
				setLoading(
					approveBtn,
					false,
					'<i class="fa-solid fa-check"></i> Phê duyệt & Hoàn tiền',
				);
		...
		rejectBtn.onclick = async () => {
			window.showConfirm(
				"Bạn chắc chắn muốn TỪ CHỐI yêu cầu hủy này? Đơn hàng sẽ quay lại trạng thái Đã xác nhận.",
				async () => {
					...
					try {
						...
						if (response.ok) {
							window.showToast(
								"Đã từ chối yêu cầu hủy. Đơn hàng quay lại trạng thái Đã xác nhận.",
								"success",
							);
							setTimeout(() => window.location.reload(), 1500);
						} else {
...
function setupInvoiceLogic(data) {
	...
	btnPrint.onclick = () => {
		...
		setElText(
			"inv-total-text",
			numberToVietnameseText(data.total_price) + " đồng chẵn.",
		);
		setElText(
			"inv-payment-status",
...
function renderActivityHistory(data) {
	...
	const history = [
		{
			...
			action: "Khách hàng khởi tạo đặt tour",
			type: "info",
		},
	];
	...
	if (data.payment_status !== "pending" || data.status === "confirmed") {
		history.push({
			time: data.created_at,
			action: "Thanh toán thành công qua Chuyển khoản",
			type: "success",
		});
	}
	...
	if (data.status === "pending" && data.payment_status === "pending") {
		history.push({
			...
			action: "Khách hàng đã gửi yêu cầu hủy tour",
			type: "warning",
		});
	}
	...
	if (data.status === "confirmed") {
		history.push({
			time: data.updated_at,
			action: "Hệ thống xác nhận booking",
			type: "success",
		});
	}

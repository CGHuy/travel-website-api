...
async function fetchAndRenderTours() {
    ...
    try {
        const payload = await fetchJson(TOUR_API_URL, { method: "GET" }, false);
        ...
        updateTotalBadge(adminTourImageCache.length);
    } catch (error) {
...
function renderTourImageCards(tours) {
    ...
    tours.forEach((tour) => {
        ...
        const code = String(tour.tour_code || `TOUR${String(id).padStart(3, "0")}`);
        ...
        if (countEl) countEl.textContent = `Số ảnh phụ: ${images.length}`;
        if (openBtn) {

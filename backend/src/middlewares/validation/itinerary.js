// Validation cho tạo/cập nhật itinerary
exports.validateItinerary = (req, res, next) => {
    const { day_number, description } = req.body;
    const errors = [];

    if (!day_number || isNaN(day_number) || parseInt(day_number) < 1) {
        errors.push("Số ngày phải là số dương");
    } else if (parseInt(day_number) > 365) {
        errors.push("Số ngày không được vượt quá 365");
    }
...
exports.validateBulkItineraries = (req, res, next) => {
    ...
    if (!Array.isArray(days)) {
    ...
    } else if (days.length > 365) {
        errors.push("Lịch trình không được vượt quá 365 ngày");
    } else {
        ...
        days.forEach((day, index) => {
            ...
            } else if (parseInt(day.day_number) > 365) {
                errors.push(`Ngày ${index + 1}: số ngày không được vượt quá 365`);
            }

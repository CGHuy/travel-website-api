...
class TourService {
	...
	static async getByTourId(tour_id) {
		try {
			const [rows] = await db.query(
				`SELECT * FROM tour_services WHERE tour_id = ?`,
				[tour_id],
			);
			return rows;
		} catch (error) {
	...
	static async getByServiceId(service_id) {
		try {
			const [rows] = await db.query(
				`SELECT * FROM tour_services WHERE service_id = ?`,
				[service_id],
			);
			return rows;
		} catch (error) {
	...
	static async getById(id) {
		try {
			const [rows] = await db.query(
				`SELECT * FROM tour_services WHERE id = ?`,
				[id],
			);
			return rows[0];
		} catch (error) {
	...
	static async delete(id) {
		try {
			const [result] = await db.query(
				`DELETE FROM tour_services WHERE id = ?`,
				[id],
			);
	...
	static async deleteByTourId(tour_id) {
		try {
			const [result] = await db.query(
				`DELETE FROM tour_services WHERE tour_id = ?`,
				[tour_id],
			);
	...
	static async deleteByServiceId(service_id) {
		try {
			const [result] = await db.query(
				`DELETE FROM tour_services WHERE service_id = ?`,
				[service_id],
			);
			return result.affectedRows;
		} catch (error) {
	...
	static async getServicesByTourIdForListTour(tour_id) {
		try {
			const [rows] = await db.query(

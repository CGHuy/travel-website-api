...
const TEST_CASE = "TC_AUTH_LOGIN_01";
const TEST_NAME = TEST_CASE + " - Đăng nhập customer thành công";
...
(async () => {
    ...
    try {
        ...
        log("Bước 1: Nhập tài khoản customer (tuan@gmail.com / 123456)");
        await page.type("#username", "tuan@gmail.com");
        await page.type("#password", "123456");

        await safeScreenshot(page, "01_before_submit.png");

        log("Bước 2: Submit form đăng nhập");
        const [loginResponse] = await Promise.all([page.waitForResponse((response) => response.url().includes("/api/auth/login") && response.request().method() === "POST"), page.click("button[type='submit']")]);
        ...
        try {
            const uiMessage = await page.$eval("#loginMessage", (el) => el.textContent.trim());
        ...
        const testPassed = loginResponse.ok() && responseData.success === true && messageText.includes("Đăng nhập") && messageText.includes("thành công") && Boolean(token) && Boolean(user);
